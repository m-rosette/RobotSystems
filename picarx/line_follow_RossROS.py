...
from gray_controller import Controller
...
class RossROS_Exexute():
    def __init__(self) -> None:
        self.px = Picarx()
        self.gray_sensor = GraySensor()
        self.gray_interpreter = Interpreter()
        self.gray_controller = Controller(self.px)
    ...
    def execute(self):
        while True:
            ...
            interpreter_cons_prod = ConsumerProducer(self.gray_interpreter, input_buses=self.grayscale_bus, output_buses=self.interp_bus, delay=0.1, termination_buses=self.terminator_bus, name="interpreter consumer producer")
            controller_consumer = Consumer(self.gray_controller, input_buses=(self.interp_bus, self.ultrasonic_bus), delay=0.1, termination_buses=self.terminator_bus, name="sensor producer")
